...
void *msrp_recv_thread(void *data)
{
	...
	while(msrp_exists) {
		tempfds = recv_fds;
		err = select(recv_fdmax+1, &tempfds, NULL, NULL, (struct timeval *)NULL); // timeval==NULL, so is block
		...
		if(err < 0)
			...
			for(fd = 0; fd <= recv_fdmax; fd++) {
				if(FD_ISSET(fd, &tempfds)) {  //if have something to read (in this method, it does not consider the write)
...
//								}
								...
								if(session->to) {
									...
									local_events(MSRP_LOG, "Unexpected connection from from %s:%hu (fd %d)",
										inet_ntoa(client_address.sin_addr) ? inet_ntoa(client_address.sin_addr) : "???.???.???.???",
										ntohs(client_address.sin_port), client);
									close(client); //make be must change in windows   ----wfh
								}
							}
						} else {
...
void msrp_recv_add_fd(int fd)
{
	...
	err = send(recv_pair[1], &ch, 1, 0); //why ???
	if(err < 1)
