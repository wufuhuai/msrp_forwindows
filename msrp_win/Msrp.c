...
int msrp_init(void(*app_events)(int event, void *info))
{
	...
	MSRP_LIST_SETUP(contexts, contexts_lock);  //???msrp_context_new() ÄÚÈÝ
...
#ifdef USEFORWIN
	MSRP_CONTEXT_LIST_ADD_WIN(contexts, contexts_lock, context, -1);
#else
...
MsrpEndpoint *msrp_endpoint_new(void)
{
	...
	session = msrp_session_new(endpoint->ID);
	if(!session) {
...
#ifdef USEFORWIN
	MSRP_SESSION_LIST_ADD_WIN(sessions, sessions_lock, session, -1);
#else
	MSRP_LIST_ADD(contexts, contexts_lock, context, -1);
#endif
...
int msrp_endpoint_set_from(MsrpEndpoint *endpoint, char *address, unsigned short int port, int content, int flags, int rights)
{
	...
	session = (msrp_session *)endpoint->endpoint;
	if(!session) {
...
int msrp_endpoint_set_to(MsrpEndpoint *endpoint, const char *path, int content, int flags, int rights)
{
	...
	if(!pnt)	/* Invalid path */
		return -1;
	path += pos;
	pnt = strstr(path, ":"); /*find the addr (the : is before the port)*/
	if(!pnt)
		return -1;
